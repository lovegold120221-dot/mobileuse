...
class ScreenAutomationService {
  ...
  Future<String> getScreenDescription() async {
    ...
    for (final node in nodes) {
      ...
      if (node['bounds'] != null) {
        ...
        boundsStr =
            ' bounds:[${b['left']},${b['top']},${b['right']},${b['bottom']}] center:(${centerX.round()},${centerY.round()})';
      }
  ...
  Future<String> getCompressedScreenDescription(String task) async {
    ...
    for (final node in nodes) {
      ...
      buffer.writeln(
        '[$index]$targetMark $type $label $tagStr$boundsStr'.trim().replaceAll(
          RegExp(r'\s+'),
          ' ',
        ),
      );
    ...
    developer.log(
      'Screen Dump Extracted:\n$screenString',
      name: 'ScreenAutomation',
    );
    return screenString;
  }

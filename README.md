...
MobileUse Agent is an open-source Android automation agent built with Flutter. It utilizes the DeepSeek API and native Android Accessibility Services to interpret screen layouts and execute multi-step tasks across any installed application via natural language commands.
...
MobileUse Agent supports Android 8.0 (API 26) and newer. Current release builds are
...
4. Launch MobileUse Agent and go to the **Settings** screen.
...
8. Enable the **"MobileUse Agent Screen Control"** service in your Android Accessibility Settings.
...
1. Open **Settings → Apps → MobileUse Agent**.
2. Open the three-dot menu in the top-right corner.
3. Tap **Allow restricted settings** and confirm.
4. Return to MobileUse Agent and open **Accessibility Settings** again.
5. Enable **MobileUse Agent Screen Control**.

MobileUse Agent now shows these instructions and provides shortcuts to both App
...
2. Input the token in the MobileUse Agent Settings screen and enable the integration toggle.

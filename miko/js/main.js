...
async function triggerMikoSpeech() {
  const message = "Hello, I'm Miko. How can I assist you today?"
  logMessage(message, "user")
...
const video = document.getElementById("miko-video")
const defaultVideoSrc = "miko.mp4"
const listeningVideoSrc = "listening.mp4"
let currentVideoMode = "default"
let listeningLoopEnd = null
const listeningLoopStart = 0.08
const listeningLoopPadding = 0.08

function setVideoSource(src, loop = true) {
  if (!video) return
  const fullSrc = new URL(src, window.location.href).href
  if (video.currentSrc === fullSrc) return
  video.src = src
  video.muted = true
  video.loop = loop
  listeningLoopEnd = null
  video.load()
  video.play().catch(() => {})
}

function setVideoMode(mode) {
  if (!video || currentVideoMode === mode) return
  currentVideoMode = mode
  if (mode === "listening") {
    setVideoSource(listeningVideoSrc, false)
  } else {
    setVideoSource(defaultVideoSrc, true)
  }
}
...
if (video) {
  ...
  video.addEventListener("loadedmetadata", () => {
    if (video.src.includes(listeningVideoSrc) && video.duration > listeningLoopStart + listeningLoopPadding) {
      listeningLoopEnd = Math.max(video.duration - listeningLoopPadding, listeningLoopStart + 0.05)
      if (video.currentTime < listeningLoopStart || video.currentTime > listeningLoopEnd) {
        video.currentTime = listeningLoopStart
      }
    } else {
      listeningLoopEnd = null
    }
  ...
  video.addEventListener("timeupdate", () => {
    if (currentVideoMode === "listening" && listeningLoopEnd != null && video.currentTime >= listeningLoopEnd) {
      video.currentTime = listeningLoopStart
      video.play().catch(() => {})
    }
  })
